...
@section('page_header')
    ...
    @if (Auth::user()->role_id == '1' or Auth::user()->role_id == '2' )
      {{-- <h1>usuario :{{ Auth::user()->role_id }}</h1> --}}
      <div class="container-fluid">
          <h1 class="page-title">
              <i class="{{ $dataType->icon }}"></i> {{ $dataType->display_name_plural }}
          </h1>
    ...
    @else
      {{-- <h1>usuario no es 1 , es:{{ Auth::user()->roles_id }}</h1> --}}
      <?php //$diferente_user = ($dataTypeContent->where('author_id','=', Auth::user()->id) as $data); ?>
      @can('add',app($dataType->model_name))
          <a href="{{ route('voyager.'.$dataType->slug.'.create') }}" class="btn btn-success btn-add-new">
              <i class="voyager-plus"></i> <span>{{ __('voyager::generic.add_new') }}</span>
          </a>
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    {{-- @foreach($dataTypeContent->where('author_id','=', Auth::user()->id) as $data) --}}
                                      {{-- @foreach($dataTypeContent->where('author_id','=', Auth::user()->role_id) as $data) --}}
                                      {{-- @if ($_SESSION['role'] == 'Desarrollador') --}}
                                    ...
                                    {{-- @if ($_SESSION['role'] == 'Desarrollador')
                                      @foreach($dataTypeContent as $data)
                                    @else
                                      @foreach($dataTypeContent->where(Auth::user()->id->roles->contains('Desarrollador')) as $data)
